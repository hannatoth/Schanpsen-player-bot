    ...
    while not state.finished():
        ...
        if is_valid(move, player):  # check for common mistakes
            ...
            if move[0] is None:
                ...
                pr('*   Player {} plays: {}{}'.format(state.whose_turn(), util.get_rank(move[0]),
                                                      util.get_suit(move[0])), verbose)

                if move[1] is not None:
                    pr('*   Player {} melds a marriage between {}{} and {}{}'.format(state.whose_turn(),
                                                                                     util.get_rank(move[0]),
                                                                                     util.get_suit(move[0]),
                                                                                     util.get_rank(move[1]),
...
def get_move(state, player, max_time, verbose):
    ...
    # Start a process with the function 'call_player' and the given arguments We also give it the state of the global
    # PRNG to ensure execution is deterministic whenever no timeouts happen we make a call to random to ensure that
    # next invocations of get_move will give start with a different state of the rng
    random.random()
...
def is_valid(
    ...
    if ((type(move[0]) is not int) and (move[0] is not None)) or ((type(move[1]) is not int) and (move[1] is not None)):
        print('Bot {} returned a move {} that was not a tuple for which each element is either an int or None'.format(
            player, move))
        return False
